    ...
    */
    public function getTabCounts($codes = null, $dummy = null) {
        ...
        foreach($codes as $locale => $name) {
            ...
            $navtab_counts['apiapp'][$locale] = $this->find('count', array(
                'conditions' => array(
                    'Token.deleted' => 0,
                    'Token.locale' => $locale,
                    'Token.instance' => 'apiapp'
                )
            ));

            $navtab_counts['widgetapp'][$locale] = $this->find('count', array(
                'conditions' => array(
                    'Token.deleted' => 0,
                    'Token.locale' => $locale,
                    'Token.instance' => 'widgetapp'
                )
            ));
        }
        ...
        $navtab_counts['apiapp']['overall'] = 0;
        foreach($navtab_counts['apiapp'] as $locale => $count) {
            $navtab_counts['apiapp']['overall'] += $count;;
        }

        $navtab_counts['widgetapp']['overall'] = 0;
        foreach($navtab_counts['widgetapp'] as $locale => $count) {
            $navtab_counts['widgetapp']['overall'] += $count;;
        }
