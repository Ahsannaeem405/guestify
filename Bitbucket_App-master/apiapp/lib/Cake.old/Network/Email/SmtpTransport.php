 ...
 */
	protected function _smtpSend($data, $checkCode = '250') {
		...
		while ($checkCode !== false) {
			...
			# check for the Amazon SES mail message id
			if($checkCode == '250') {
				$end_line = end($responseLines);
				if(substr_count($end_line, '-') == 6) {
					$this->mail_message_id = str_replace('250 Ok ', '', $end_line);
					#CakeLog::write('smtp', $this->mail_message_id);
				}
			}
