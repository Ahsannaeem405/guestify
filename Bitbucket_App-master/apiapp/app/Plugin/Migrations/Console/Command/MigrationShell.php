...
App::import('Vendor', 'Yaml', array('file' => '..' . DS . 'Plugin' . DS . 'Migrations' . DS . 'vendors' . DS . 'Yaml.php'));
...
class MigrationShell extends AppShell
{
    ...
    */
    ...
    function startup()
    {
        ...
        #if (preg_match("/\/plugins\/Migrations\/Console\/Command\/migrate\.php$/", $this->Dispatch->shellPath)) {
        ...
        define('MIGRATIONS_PATH', ROOT . DS . APP_DIR . DS . 'Config' .DS. 'migrations');
   ...
   */
    ...
    */
    ...
    function generate()
    {
        ...
        if (isset($this->args[0]) && ($this->args[0] == 'at')) {
            $name = '';
        }
    ...
    function _initDatabase()
    {
        ...
        if(isset($this->args[0])) {
            ...
            if(in_array($arg, array('local', 'stage', 'live'))) {
                $source = $this->args[0];
            }
        }

        $config = (array)ConnectionManager::getDataSource($source);

        $dsn = array(
            'phptype'   =>  "mysql",
            'username'  =>  $config['config']['login'],
            'password'  =>  $config['config']['password'],
            'hostspec'  =>  $config['config']['host'],
            'database'  =>  $config['config']['database'],
            'charset'  =>   $config['config']['encoding']
        );
        $options = array(
            'debug'         =>  'DEBUG',
        ...
        $this->_db = $MDB2->connect($dsn, $options);     // seems to make problems!
    ...
    {
        $this->out('');
        $this->out($this->_colorize('  ** '.$message.' **', 'ERROR'));
        $this->out('');
        $this->hr();
        $this->out('');
        exit;
